ALTER TABLE public.shadow_shifts
  ...
  ADD COLUMN IF NOT EXISTS dress_group text;

CREATE OR REPLACE FUNCTION public.create_shadow_shift(p_person_id uuid, p_role text, p_shift_date date, p_arrival_time time without time zone, p_trainer_person_id uuid DEFAULT NULL::uuid, p_note text DEFAULT NULL::text, p_section text DEFAULT NULL::text, p_dress_group text DEFAULT NULL::text)
...
BEGIN
  ...
  INSERT INTO public.shadow_shifts
    (owner_id, person_id, role, shift_date, arrival_time, trainer_person_id, note, section, dress_group)
  VALUES
...
CREATE OR REPLACE FUNCTION public.update_shadow_shift(p_id uuid, p_shift_date date, p_arrival_time time without time zone, p_trainer_person_id uuid DEFAULT NULL::uuid, p_note text DEFAULT NULL::text, p_section text DEFAULT NULL::text, p_dress_group text DEFAULT NULL::text)
 RETURNS shadow_shifts
 LANGUAGE plpgsql
 SECURITY DEFINER
 SET search_path TO 'public'
...
BEGIN
  ...
  -- or decline. Recomputed section / dress group must never clear it.
